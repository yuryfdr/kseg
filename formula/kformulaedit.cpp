...
void KFormulaEdit::redraw_all(int all,QPainter&p)
{
  //QPainter p;
...
{
    if (!restricted) ; //bad things happen if it's not restricted.
...
#define MODIFIED { form->parse(formText, &info); CURSOR_RESET \
  cacheState = ALL_DIRTY; undo_stack.push( ((QString(oldText)). \
...
{
  int shift = (e->state() & Qt::ShiftModifier); //easier to type "shift"
  ...
  if(e->key() == Qt::Key_Down) { // move down a cell in a matrix
  ...
  if(e->key() == Qt::Key_Up) { // move to the cell above
    int m = isInMatrix();
  ...
  if(e->key() == Qt::Key_Home ||
     (e->state() & Qt::ControlModifier && e->key() == Qt::Key_A)) {
    if(cursorPos != 0) {
  ...
  if(e->key() == Qt::Key_End ||
     (e->state() & Qt::ControlModifier && e->key() == Qt::Key_E)) {
  ...
  if(e->state() & Qt::ControlModifier) {
    ...
    if(e->key() == Qt::Key_Z) { // undo
      do_undo(oldText, oldc);
      return;
    }

    if(e->key() == Qt::Key_R) { // redo
  ...
  if(!(e->state() & (Qt::ControlModifier | Qt::AltModifier))  && e->ascii() >= 32 &&
     !strchr("{})]#", e->ascii())) {
  ...
  if(e->state() & Qt::ControlModifier) {
    ...
    case Qt::Key_3: // the matrix--insertChar handles the dialog
...
void KFormulaEdit::do_undo(QString oldText, int oldc)
{
  ...
  if(!undo_stack.isEmpty()) {
    ...
    redo_stack.push(QString(oldText).insert(oldc, QChar(CURSOR)) );
    formText = undo_stack.pop();
...
void KFormulaEdit::do_redo(QString oldText, int oldc)
{
  ...
  if(!redo_stack.isEmpty()) {
    ...
    undo_stack.push(QString(oldText).insert(oldc, QChar(CURSOR)) );

    formText = redo_stack.pop();
...
void KFormulaEdit::insertChar(QChar c)
{
  ...
  if(restricted) { // we need to limit to only those things
    ...
    if(!/*isalnum(c.cell())*/c.isDigit() && !c.isSymbol() && !c.isLetter() && !c.isSpace() && c !=QChar('.') && !IS_REFERENCE(c.unicode()) &&
       !(KFormula::eval()).contains(c) && (extraChars.isNull() || !extraChars.contains(c))){
       return;
    }
  }
