...
QString KFormula::toUgly(QString ugly)
{
  ...
  r.setPattern(QString(L_GROUP) + R_GROUP + QChar(PAREN)); //parentheses
  ugly.replace(r, "");  // {}({...} --> {...}

  //r = QString(L_GROUP) + R_GROUP + QChar(ABS); // absolute value
  r.setPattern(QString(L_GROUP) + R_GROUP + QChar(ABS)); // absolute value
...
double KFormula::evaluate(const QMap<QString,double>& variables,
			  int *error, box *b)
{
  ...
  *error = NO_ERROR;
  ...
  if(b->type == TEXT) {
    ...
    if(temptext.length() > 0) { //is it empty?
      ...
      //std::cerr<<variables.size()<<" "<<temptext.toStdString()<<" "<<variables.autoDelete()<<std::endl;
      QMap<QString,double>::const_iterator v = variables.find(temptext);
      if(v != variables.end()) return *v;
...
void KFormula::parse(QString text, QVector<charinfo> *info)
{
  int i, j;
...
// i = 6.  After: "{x}+{{y}*{3}}"
void KFormula::parenthesize(QString &temp, int &i, QVector<charinfo> *info)
{
  int j;
  int level = 0;
  int need_paren = 0;
...
box * KFormula::makeBoxes(QString str, int offset,
			  int maxlen, QVector<charinfo> *info)
{
