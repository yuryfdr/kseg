...
box::~box()
{
  if(!IS_REFERENCE(type)) {
    ...
    if(b1 != NULL) b1->refParents.removeAll(this);//(b1->refParents.indexOf(this));//removeRef(this);
  }
  b1 = b2 = NULL;
  if(parent) {
...
void box::calculate(QPainter &p, int setFontsize, QFont *f_, QColor *, QColor *)
{
  ...
  //QFontInfo font_info_tmp = p.fontInfo();
  ...
  switch(type)
	...
	rect = fm.boundingRect(text);
      ...
      { // fix the height of the box to not depend on the letters
	QRect heightRect = fm.boundingRect("X");
	rect.setTop(heightRect.top());
...
void box::draw(QPainter &p, int x, int y, QFont *f_, QColor *bc, QColor *fc)
{
  ...
  case MORE:
  case LESS:
    p.drawText(x + relx, y + rely, QChar(type)); //just draw the operator
    break;
